...
class GradientImportanceScoreEvaluator(BaseImportanceScoreEvaluator):
    ...
    def evaluate(self, input_ids: torch.Tensor, target_id: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self.model, GPT2LMHeadModel):
            ...
            word_token_embeds = llamaModel.model.embed_tokens(input_ids)
            pos_encoded_embeddings = word_token_embeds
        elif isinstance(self.model, Qwen2ForCausalLM):
            qwen2Model: Qwen2ForCausalLM = self.model
            word_token_embeds = qwen2Model.model.embed_tokens(input_ids)
            pos_encoded_embeddings = word_token_embeds
        else:
        ...
        if self.grad_type == 'integrated_gradients':
            ...
            all_gradients = []
            for i in range(1, path_integral_steps + 1):
